/*************************************************************************
	...
	> Created Time: 2023年05月04日 星期四 16时58分07秒
 ************************************************************************/
...
HuffmanNode* buildHuffmanTree(string str){
	// 统计字符的频率
	unordered_map<uint8_t, int> freq_map;
	for(uint8_t c : str){
		freq_map[c]++;
	}
	// 构建小根堆
...
// 递归实现哈夫曼编码
void encodeHelper(HuffmanNode* root, string code, unordered_map<uint8_t, string>& code_map){
	if(root == nullptr){
		return;
	}
...
// 哈夫曼编码
unordered_map<uint8_t, string> huffmanEncode(string str){
	unordered_map<uint8_t, string> code_map; // 存储字符对应的编码
